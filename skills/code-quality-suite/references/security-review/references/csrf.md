...
Use a cryptographically signed token that does not require server-side storage.
...
For AJAX/API requests, require a custom header that cannot be set cross-origin without CORS.
