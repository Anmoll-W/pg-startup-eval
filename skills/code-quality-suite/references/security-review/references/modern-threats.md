...
const url = window.location.href;  // Cannot be clobbered
...
function safeGetElement(name) {
    ...
    return null;  // It is a DOM element, not expected object
}
...
def validate_llm_output(output, expected_format):
    ...
    if not matches_expected_format(output, expected_format):
        raise InvalidOutput("Output does not match expected format")
