...
- `chart-type` - Match chart type to data type (line for trend, bar for comparison, pie or donut for proportion)
...
| **Fix a UI bug** | "Button hover is broken", "Layout shifts on load" | Quick Reference, then relevant section |
...
- Try different keywords for the same need: `"playful neon"`, then `"vibrant dark"`, then `"content-first minimal"`
- Use `--design-system` first for full recommendations, then `--domain` to deep-dive any dimension you are unsure about
...
| Problem | What to Do |
|---------|------------|
| Cannot decide on style/color | Re-run `--design-system` with different keywords |
...
- Run through Quick Reference **§1 to §3** (CRITICAL + HIGH) as a final review
- Test on 375px (small phone) and landscape orientation
- Verify behavior with **reduced-motion** enabled and **Dynamic Type** at largest size
- Check dark mode contrast independently (do not assume light mode values work)
