...
| a16z | Counter-positioning lens: structural vs. executional reasons incumbents cannot copy you |
...
3. **The Thiel Secret**: what the founder knows that most people do not; flagged as absent if not articulable
...
cp -r thepmcode-skills/skills/pg-startup-eval ~/.claude/skills/pg-startup-eval
```

That is it. Claude Code auto-discovers skills in `~/.claude/skills/`: no settings file changes needed.
...
- **Pivot Required**: the idea as stated will not work; proposes one specific adjacent idea worth exploring based on the founder's domain and the customer pain that was identified
