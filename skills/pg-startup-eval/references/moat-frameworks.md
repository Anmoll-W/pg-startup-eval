...
- **Indirect (cross-side)**: Two distinct user groups benefit from each other's participation. Marketplaces, platforms. The flywheel is: more buyers leads to more sellers leads to better prices/selection leads to more buyers.
- **Data network effects**: More users generate more data leads to better ML model leads to better product leads to more users. This is *not* automatic: it requires the feedback loop to actually improve the product for users. Many companies claim this but the loop is too slow or too noisy to matter.
...
- "They're slower" is not structural (they can hire/acquire)
- "They don't care about this segment" is slightly structural (requires deliberate neglect)
- "Their pricing model requires per-seat contracts; our usage-based model would cannibalize 40% of their existing ARR" is structural (changing requires self-harm)
- "They are the incumbent that our customers are trying to escape; being associated with them is a liability" is structural (brand is working against them)
