...
7. `<examples>`: 1–2 concise input to output demonstrations. Required for any task where format or judgment could be ambiguous.
...
- If the input is clear enough to determine domain, deliverable, and audience, then proceed directly to prompt generation.
- If the input is critically ambiguous (e.g., "writer" with no domain, audience, or output type), then ask at most 2 targeted clarifying questions before generating.
